...
static BOOT_PTABLE: PageTable = {
    let mut ptable: PageTable = [0;PTABLE_LENGTH];
    let ptablentry: PageTableEntry = create_pde!(KERNEL_PAGE_ADDR, PTableEntryFlags::RWX);
    ptable[KERNEL_ENTRY_VPN2] = ptablentry;  // 1GiB page
    ptable[KERNEL_SPACE_VPN2 + KERNEL_ENTRY_VPN2] = ptablentry;  // higher half 1GiB page
    ptable
};

...
fn main(){
    ...
    let flags_str = fs::read_to_string(PathBuf::from(manifest_dir.clone()).join("link_flags.json")).unwrap();
...
fn make_linker(entry:u64, space:u64){
    ...
    let mut sout = fs::read_to_string(PathBuf::from(manifest_dir.clone()).join("linker.cross.ld")).unwrap();
